...
KIARA.ConstructionPlan.prototype.start = function(gameState)
{
	...
	if (!builder)
	{
		KIARA.Logger.debug("KIARA error: builder not found when starting construction.");
		Engine.ProfileStop();
		this.started = true;
		return;
	}
...
KIARA.ConstructionPlan.prototype.buildOverseaDock = function(gameState, template)
{
	...
	if (!gameState.ai.HQ.navalMap)
		KIARA.Logger.debug("KIARA.findOverseaLand on a non-naval map??? we should never go there ");
